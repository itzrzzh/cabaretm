...
let g = client.guilds.get("737374111227183326"); // id server
...
client.on('ready', () => {
    ...
    client.user.setActivity(`Type ${prefix}help`,{type: 'Plying'}); ///التعديل علي البلاينج
});
...
// حسن كههربا
        ...
        return msg.channel.send(':notes: تم ايقاف تشغل الصوت ومسح قائمة الإنتظار.');
// 04
    } else if(cmd === 'skip') {
      ...
      let queue = active.get(msg.guild.id);
        ...
        if(!vCh) return msg.channel.send('عذرًا ، لكن لا يمكنك ذلك لأنك لست في القناة الصوتية');

        if(!queue) return msg.channel.send('لا توجد موسيقى شغاله لتخطيها');
...
// 14
        ...
        if(req == 1) {
            msg.channel.send('**:notes: تم التخطي **' + args);
